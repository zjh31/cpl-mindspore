...
class TransVGDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        query_token = self.tokenizer(phrase, max_length=20, padding="max_length", truncation=True, return_tensors='ms')
        ...
        if self.testmode:
            ...
            # print(img.shape)
            return np.array(img, dtype=np.float32), np.array(img_mask), query_token['input_ids'].asnumpy(), query_token['attention_mask'].asnumpy(), np.array(bbox, dtype=np.float32)
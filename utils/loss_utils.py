...
import pdb
from utils.box_utils import bbox_iou, xywh2xyxy, xyxy2xywh, generalized_box_iou, ms_xywh2xyxy, ms_xyxy2xywh
...
class trans_vg_loss(nn.Cell):
    def __init__(self):
        super(trans_vg_loss, self).__init__()
    def construct(self, base, target):
        num_boxes = base.shape[0]

        loss_bbox = ops.l1_loss(base, target, reduction='none')
        loss_giou = 1 - np.diag(generalized_box_iou(
            ms_xywh2xyxy(base),
            ms_xywh2xyxy(target)
        ))

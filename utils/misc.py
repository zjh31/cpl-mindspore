...
def collate_fn(raw_batch):
    raw_batch = list(zip(*raw_batch))
    img = ms.tensor(raw_batch[0])
    #img = ops.stack(raw_batch[0])
    img_mask = ms.tensor(raw_batch[1])
    img_data = NestedTensor(img, img_mask)
    word_id = ms.tensor(raw_batch[2], dtype=ms.int32)
    word_mask = ms.tensor(raw_batch[3])
    text_data = NestedTensor(word_id, word_mask)
    bbox = ms.tensor(raw_batch[4])
    batch = [img_data, text_data, bbox]
    return tuple(batch)
def collate_fn_torch(raw_batch):
    raw_batch = list(zip(*raw_batch))
    img = ms.tensor(raw_batch[0])
    #img = ops.stack(raw_batch[0])
    img_mask = ms.tensor(raw_batch[1])

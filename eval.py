...
from models import build_model, bulid_BERT
...
from convert import param_convert, pytorch_params
import warnings
...
def get_args_parser():
    ...
    # dataset parameters
    parser.add_argument('--output_dir', default='./outputs/unc',
                        help='path where to save, empty for no saving')
    parser.add_argument('--device', default='cuda',
                        help='device to use for training / testing')
    parser.add_argument('--seed', default=13, type=int)
    parser.add_argument('--resume', default='/home/zhangjiahua/Code/TransVG/unc_best.ckpt', help='resume from checkpoint')
    ...
    # evalutaion options
    parser.add_argument('--eval_set', default='testA', type=str)
    parser.add_argument('--eval_model', default='unc_r50.ckpt', type=str)
...
def main(args):
    ...
    #bert_model = bulid_BERT(args)
    #bert_model.to(device)


    n_parameters = sum(p.numel() for p in model.get_parameters())
    print('number of params:', n_parameters)

    # build dataset
    print("build dataset!")
    dataset_test = build_dataset(args.eval_set, args)
    print("build dataset finish!")
    ...
    param_dict = ms.load_checkpoint(args.eval_model)
    ...
    # perform evaluation
    accuracy = validate(args, model, data_loader_test, device)
    ...
    if utils.is_main_process():
        ...
        log_stats = {'dataset:': args.dataset,
